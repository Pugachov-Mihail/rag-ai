...
from read_book_arch import async_knowledge_crawler, rebuild_vector_db
...
def build_base_plan(user_query: str, mode: str) -> QueryPlan:
    ...
    need_arch = mode in {"consult", "analyze-project"} or any(x in text for x in [
        "архитект", "рефактор", "ddd", "cqrs", "слой", "модул", "bounded context", "event sourcing", "границ", "декомпоз"
    ])
    ...
    response_mode = "ultra-short" if ultra else (
        "plan" if task_type in {"architecture", "project-audit"} else ("diff" if need_diff else "standard")
    )
    return QueryPlan(
        task_type=task_type,
        response_mode=response_mode,
        books_top_k=4 if need_arch else 3,
        project_top_k=6 if need_diff or need_arch or task_type == "project-audit" else 4,
        arch_top_k=10 if task_type == "project-audit" else 4,
        ...
        arch_query="project architecture overview",
    )


def planner_refine(user_query: str, base_plan: QueryPlan, incoming_messages: List[Dict]) -> QueryPlan:
    planner_prompt = (
        "Ты planner для Go-помощника внутри IDE с RAG и архитектурным профилем. Не отвечай на вопрос пользователя.\n"
        "Верни JSON с полями: task_type, response_mode, books_top_k, project_top_k, arch_top_k, needs_review, need_diff, need_architecture, ultra_short, code_query, books_query, memory_query, arch_query.\n"
        "code_query оптимизируй под поиск по коду, books_query — под книги, memory_query — под память, arch_query — под архитектурный профиль (по модулям/слоям).\n"
        "Если режим analyze-project — сфокусируйся на полном обзоре архитектуры, а не на одной задаче."
    )
    ...
    try:
        ...
        return QueryPlan(
            ...
            project_top_k=max(1, min(int(data.get("project_top_k", base_plan.project_top_k)), 10)),
            arch_top_k=max(3, min(int(data.get("arch_top_k", base_plan.arch_top_k)), 20)),
            ...
            arch_query=(data.get("arch_query") or base_plan.arch_query)[:200],
        )
    except Exception:
        return base_plan
...
def render_code_blocks(items: List[Dict]) -> Tuple[str, List[str]]:
    blocks, refs = [], []
    for item in items:
...
def render_book_blocks(items: List[Dict]) -> Tuple[str, List[str]]:
    blocks, refs = [], []
    for item in items:
...
def render_arch_blocks(items: List[Dict]) -> Tuple[str, List[str]]:
    blocks, refs = [], []
    for item in items:
        ...
        public_entry_points = item.get("public_entry_points", [])
        forbidden_patterns = item.get("forbidden_patterns", [])
        change_recipe = item.get("change_recipe", "")
        refs.append(module)
        blocks.append(
            f"Модуль: {module}\nСлой: {layer}\nОтветственность: {responsibility}\n"
            f"Depends on: {depends_on}\nUsed by: {used_by}\nPublic entry points: {public_entry_points}\n"
            f"Forbidden: {forbidden_patterns}\nChange recipe: {change_recipe}"
        )
    return "\n---\n".join(blocks), refs
...
def build_context(plan: QueryPlan) -> RetrievedContext:
    ...
    arch_items = query_qdrant("project_arch_profile", plan.arch_query, plan.arch_top_k)
    memory_text = get_relevant_chat_memory(plan.memory_query)
    code_text, code_refs = render_code_blocks(code_items)
    book_text, book_refs = render_book_blocks(book_items)
    arch_text, arch_refs = render_arch_blocks(arch_items)
    return RetrievedContext(
        books=trim_text(book_text, 4000),
        project=trim_text(code_text, 9000),
        memory=trim_text(memory_text, 1200),
        arch=trim_text(arch_text, 4000),
        meta={
            "code_query": plan.code_query,
            "books_query": plan.books_query,
            "memory_query": plan.memory_query,
            "arch_query": plan.arch_query,
            "code_refs": code_refs,
            "book_refs": book_refs,
            "arch_refs": arch_refs,
        },
...
def build_system_prompt(plan: QueryPlan, context: RetrievedContext) -> str:
    if plan.task_type == "project-audit":
        contract = (
            "Сделай архитектурный аудит проекта: опиши слои, модули, allowed dependencies, типовые паттерны и точки расширения."
        )
    elif plan.ultra_short:
        contract = "Ответ строго 5-8 строк, без воды, сначала диагноз, затем действие, потом проверка."
    elif plan.response_mode == "diff":
        contract = "Дай короткий диагноз, затем минимальный Go diff/patch, затем 2-3 проверки после фикса."
    elif plan.response_mode == "plan":
        contract = "Дай диагноз, целевую архитектуру в рамках текущих модулей, шаги миграции и риски."
    else:
        contract = "Дай прикладной ответ по существу, строго в рамках текущей архитектуры проекта."

    return (
        "Ты Go-помощник внутри IDE с доступом к AST-коду, книгам и архитектурному профилю проекта.\n"
        "Порядок: пойми запрос -> опирайся на архитектурный профиль (слои/модули) -> выбери правильное место для изменений -> опирайся на код -> применяй знания из книг.\n"
        "Приоритет источников: архитектурный профиль > код > книги > память.\n"
        "Не выдумывай новые модули, слои и пакеты. Используй существующие module_name/layer.\n"
        "Если контекста не хватает, скажи это явно.\n"
        f"{contract}\n\n"
        f"АРХИТЕКТУРА ПРОЕКТА:\n{context.arch or 'Нет данных'}\n\n"
...
def build_reviewer_prompt(user_query: str, plan: QueryPlan) -> str:
    ...
    arch_rule = "Проверь, что решение не ломает архитектурный профиль (слои/модули, depends_on/used_by)."
    return (
...
def build_repair_prompt(user_query: str, review_feedback: str, plan: QueryPlan) -> str:
    ...
    return (
        ...
        "Исправь только реальные нарушения архитектуры и качества."
        f" {brevity} Не добавляй новые идеи без необходимости."
    )
...
def run_pipeline(user_query: str, incoming_messages: List[Dict], mode: str, include_debug: bool) -> Tuple[str, Dict]:
    ...
    if plan.task_type == "project-audit":
        audit_prompt = (
            "Сделай структурированный архитектурный обзор проекта по текущему профилю.\n"
            "1. Слои (ui/transport/application/domain/infra) и их ответственность.\n"
            "2. Основные модули и их роли.\n"
            "3. Allowed dependency направления (кто может зависеть от кого).\n"
            "4. Типовые паттерны и точки расширения.\n"
            "5. Как по этой архитектуре правильно добавлять новую фичу."
        )
        messages = messages + [{"role": "user", "content": audit_prompt}]
    ...
    if plan.needs_review and not silent_mode and draft.strip():
        review_feedback = call_ollama_text(
            REVIEW_MODEL,
            messages + [{"role": "assistant", "content": draft}, {"role": "user", "content": build_reviewer_prompt(user_query, plan)}],
        )
...
def health():
    return jsonify({"status": "ok", "service": "arch-agent-rag"})
...
def manual_rebuild_db():
    ...
    thread.start()
    return jsonify({"status": "rebuilding", "message": "Полная пересборка Qdrant запущена в фоне"})


if __name__ == "__main__":
    crawler = threading.Thread(target=async_knowledge_crawler, daemon=True)
    crawler.start()
    print("[🚀] Архитектурный агент RAG запущен на http://localhost:5001")
